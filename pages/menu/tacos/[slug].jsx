...
export default function FoodItem({taco}) {
    const src = urlFor(taco.image).url()
    const { addFood} = useContext(Context)
    const [Size, setSize] = useState(1)
    const [Quantity, setQuantity] = useState(1)
    const cartItemKey = `${taco._id}-${taco.size}-${Math.random().toString(36).substring(7)}`;
    ...
    const addToCart = () => {
        addFood({...taco, price: taco.price[Size], quantity: Quantity, size: Size, cartItemKey})
    ...
    return (
      <div className={css.overlay}>
          <Link className={css.left} href='/menu/tacos'><span><HiArrowLeft fontSize={15} /> Back to Tacos Menu</span></Link>
        ...
        <div className={css.container}>
          ...
          <div className={css.second}>
            ...
            <div className={css.price}>
              <span><span style={{color: "var(--clr-red)"}}>$ </span>{taco.price[Size]}</span>
            </div>
            <hr />
            <div className={css.details}>
              <span>Details</span>
              <span>{taco.details}</span>
            </div>
            ...
            <div className={css.quantity}>
              <span onClick={() =>handleQuantity('minus')}s>
                <HiMinusSm />
              </span>
              <span>{Quantity}</span>
            ...
            <div tabIndex={-1} onClick={addToCart} className={css.cta}>
              ...
              <span>{(taco.price[Size] * Quantity).toPrecision(3)}</span>
            </div>
          </div>
...
export async function getStaticPaths() {
    const paths = await client.fetch(
        `*[_type=="taco" && defined(slug.current)][].slug.current`
    )
    return {
...
// This is a function that tells nextJS to fetch the data from the taco but only fetch the specific taco data that is connected with the slug clicked on 
export async function getStaticProps(context) {
    const {slug = ""} = context.params;
    const taco = await client.fetch(
        `*[_type=="taco" && slug.current == '${slug}'][0]`
    );
    return {
        props: {taco}
    }

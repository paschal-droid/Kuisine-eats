...
export default function FoodItem({pizza}) {
    ...
    const cartItemKey = `${pizza._id}-${pizza.size}-${Math.random().toString(36).substring(7)}`;
    ...
    const addToCart = () => {
        addFood({...pizza, price: pizza.price[Size], quantity: Quantity, size: Size, cartItemKey})
    ...
    return (
      <div className={css.overlay}>
          <Link className={css.left} href='/menu/pizzas'><span><HiArrowLeft fontSize={15} /> Back to Pizza Menu</span></Link>
        ...
        <div className={css.container}>
          ...
          <div className={css.second}>
            <div className={css.foodName}>
              ...
              <span>Pizzas Menu</span>
            </div>
            <div className={css.price}>
              <span><span style={{color: "var(--clr-red)"}}>$ </span>{pizza.price[Size]}</span>
            </div>
            <hr />
            ...
            <div tabIndex={-1} onClick={addToCart} className={css.cta}>
              ...
              <span>{(pizza.price[Size] * Quantity).toPrecision(3)}</span>
            </div>
          </div>

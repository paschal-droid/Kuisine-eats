...
export default function FoodItem({snack}) {
    const src = urlFor(snack.image).url()
    const { addFood} = useContext(Context)
    const [Size, setSize] = useState(1)
    const [Quantity, setQuantity] = useState(1)
    const cartItemKey = `${snack._id}-${snack.size}-${Math.random().toString(36).substring(7)}`;
    ...
    const addToCart = () => {
        addFood({...snack, price: snack.price[Size], quantity: Quantity, size: Size, cartItemKey})
    ...
    return (
      <div className={css.overlay}>
          <Link className={css.left} href='/menu/snacks'><span><HiArrowLeft fontSize={15} /> Back to Snacks Menu</span></Link>
          <Link className={css.right} href='/cart'><span><HiArrowRight fontSize={15} /> My Cart</span></Link>
        <div className={css.container}>
          ...
          <div className={css.second}>
            ...
            <div className={css.price}>
              <span><span style={{color: "var(--clr-red)"}}>$ </span>{snack.price[Size]}</span>
            </div>
            <hr />
            <div className={css.details}>
              <span>Details</span>
              <span>{snack.details}</span>
            </div>
            <div className={css.size} >
              <div onClick={() => setSize(0)} className={Size === 0 ? css.selected : ''}>
                <GiPopcorn fontSize={25}  />
                <span>L</span>
              </div>
            ...
            <div tabIndex={-1} onClick={addToCart} className={css.cta}>
              ...
              <span>{(snack.price[Size] * Quantity).toPrecision(3)}</span>
            </div>
          </div>
...
export async function getStaticPaths() {
    const paths = await client.fetch(
        `*[_type=="snacks" && defined(slug.current)][].slug.current`
    )
    return {
...
// This is a function that tells nextJS to fetch the data from the snack but only fetch the specific snack data that is connected with the slug clicked on 
export async function getStaticProps(context) {
    const {slug = ""} = context.params;
    const snack = await client.fetch(
        `*[_type=="snacks" && slug.current == '${slug}'][0]`
    );
    return {
        props: {snack}
    }

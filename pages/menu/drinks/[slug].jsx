...
export default function FoodItem({drink}) {
    const src = urlFor(drink.image).url()
    const { addFood} = useContext(Context)
    const [Size, setSize] = useState(1)
    const [Quantity, setQuantity] = useState(1)
    const cartItemKey = `${drink._id}-${drink.size}-${Math.random().toString(36).substring(7)}`;
    ...
    const addToCart = () => {
        addFood({...drink, price: drink.price[Size], quantity: Quantity, size: Size, cartItemKey})
    ...
    return (
      <div className={css.overlay}>
          <Link className={css.left} href='/menu/drinks'><span><HiArrowLeft fontSize={15} /> Back to Drinks Menu</span></Link>
        ...
        <div className={css.container}>
          ...
          <div className={css.second}>
            ...
            <div className={css.price}>
              <span><span style={{color: "var(--clr-red)"}}>$ </span>{drink.price[Size]}</span>
            </div>
            <hr />
            ...
            <div tabIndex={-1} onClick={addToCart} className={css.cta}>
              ...
              <span>{drink.price[Size] * Quantity.toPrecision(3)}</span>
            </div>
          </div>
...
// This is a function that tells nextJS to fetch the data from the drink but only fetch the specific drink data that is connected with the slug clicked on 

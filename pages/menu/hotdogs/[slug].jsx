...
export default function FoodItem({hotdog}) {
    const src = urlFor(hotdog.image).url()
    const {addFood} = useContext(Context)
    const [Size, setSize] = useState(1)
    const [Quantity, setQuantity] = useState(1)
    const cartItemKey = `${hotdog._id}-${hotdog.size}-${Math.random().toString(36).substring(7)}`;
    ...
    const addToCart = () => {
        addFood({...hotdog, price: hotdog.price[Size], quantity: Quantity, size: Size, cartItemKey})
    ...
    return (
      <div className={css.overlay}>
          <Link className={css.left} href='/menu/hotdogs'><span><HiArrowLeft fontSize={15} /> Back to Hotdog Menu</span></Link>
        ...
        <div className={css.container}>
          ...
          <div className={css.second}>
            <div className={css.foodName}>
              <span>{hotdog.name}</span>
              <span>Hotdog Menu</span>
            </div>
            <div className={css.price}>
              <span><span style={{color: "var(--clr-red)"}}>$ </span>{hotdog.price[Size]}</span>
            </div>
            <hr />
            <div className={css.details}>
              <span>Details</span>
              <span>{hotdog.details}</span>
            </div>
            <div className={css.size} >
              <div onClick={() => setSize(0)} className={Size === 0 ? css.selected : ''}>
                <CiHotdog fontSize={25}  />
                <span>L</span>
              </div>
            ...
            <div tabIndex={-1} onClick={addToCart} className={css.cta}>
              ...
              <span>{(hotdog.price[Size] * Quantity).toPrecision(3)}</span>
            </div>
          </div>
...
export async function getStaticPaths() {
    const paths = await client.fetch(
        `*[_type=="hotdog" && defined(slug.current)][].slug.current`
    )
    return {
...
// This is a function that tells nextJS to fetch the data from the hotdog but only fetch the specific hotdog data that is connected with the slug clicked on 
export async function getStaticProps(context) {
    const {slug = ""} = context.params;
    const hotdog = await client.fetch(
        `*[_type=="hotdog" && slug.current == '${slug}'][0]`
    );
    return {
        props: {hotdog}
    }
